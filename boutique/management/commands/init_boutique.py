...
class Command(BaseCommand):
    help = 'Initialise les données de démonstration pour la boutique'

    def add_arguments(self, parser):
        parser.add_argument(
            '--confirm',
            action='store_true',
            help='Confirme l\'initialisation sans demander de confirmation',
        )

    def handle(self, *args, **options):
        if not options['confirm']:
            confirm = input('Voulez-vous initialiser les données de démonstration ? (oui/non): ')
            if confirm.lower() not in ['oui', 'yes', 'y', 'o']:
                self.stdout.write(self.style.WARNING('Initialisation annulée.'))
                return
        ...
        for cat_data in categories_data:
            ...
            self.stdout.write(f"  - {cat_data['nom']}: {status}")
        ...
        for article_data in articles_data:
            ...
            self.stdout.write(f"  - {article_data['nom']}: {status}")
        ...
        self.stdout.write(f"  - Articles en vedette : {Article.objects.filter(en_vedette=True).count()}")
        self.stdout.write(f"  - Articles en promotion : {Article.objects.filter(prix_promo__isnull=False).count()}")
